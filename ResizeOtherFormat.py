...
def process_files(input_folder, output_folder):
    ...
    for root, _, files in os.walk(input_folder):
        for file in files:
            ...
            try:
                # Only open image for timestamp extraction for supported formats here
                if ext in ['.HEIC', '.HEIF', '.PNG', '.MOV', '.heic', '.heif', '.png', '.mov']:
                    if ext != '.mov':
                        img_for_timestamp = Image.open(file_path)
                    else:
                        # For MOV, timestamp extraction is different
                        img_for_timestamp = None
                    timestamp, source_tag = (get_capture_datetime(img_for_timestamp)
                                            if img_for_timestamp else (None, None))
                else:
                    # Unsupported format - skip and log
            ...
            try:
                if ext in ['.heic', '.heif']:
                    ...
                    # Safety fallback, should not be triggered due to above check
                    print(f"Unsupported file type encountered during processing: {file_path}")
                    log_capture_time(file_path, None, 'Skipped', 'Unsupported Format')
                    continue
