...
def save_jpeg_under_size(image: Image.Image, output_path: Path, max_bytes: int):
    """
    Save image as JPEG adjusting quality to fit under max_bytes, keeping aspect ratio.
    Returns (success: bool, quality_used: int, final_size_bytes: int).
    """
    # Convert mode to RGB if not already (JPEG requirement)
    ...
    # If can't get below max_bytes even at low quality, save at quality=15 anyway
    image.save(output_path, format='JPEG', quality=15)
    real_size = output_path.stat().st_size
    return False, 15, real_size


def resize_image_to_fit(image: Image.Image, max_bytes: int) -> Image.Image:
    """
    Reduce image size by resizing (keeping aspect ratio) and compressing to fit max_bytes.
    """
    orig_img = image
    orig_w, orig_h = image.size
    min_scale = 0.1  # minimum scale factor to avoid too small images
    scale = 1.0
    step = 0.9  # scale down by 10% each iteration
    best_image = image
    last_success = False
    ...
    while scale >= min_scale:
        ...
        try:
            ...
            break  # Cannot save this resized image, stop

        size = temp_buffer.tell()
        if size <= max_bytes:
            best_image = img_resized
            last_success = True
            break  # fits size requirements
        scale *= step

    if not last_success:
        best_image = img_resized  # fallback to smallest size tried
...
def process_row(row: pd.Series) -> dict:
    ...
    try:
        ...
        output_file_name = (unique_name + '.jpg').lower()  # ensure lowercase extension
        ...
        try:
            # Attempt resize and compression to fit max file size
            resized_image = resize_image_to_fit(image, MAX_FILESIZE_BYTES)
            saved_ok, quality_used, saved_size = save_jpeg_under_size(
                resized_image, output_path, MAX_FILESIZE_BYTES
            )
            info['success'] = True
            ...
            # On failure (e.g., truncated image), copy or convert original file
            err_msg = str(e)
